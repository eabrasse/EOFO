...
datafn = home+'data/nep_nb_ssp585_moave_2056_06.nc'
da = xr.load_dataset(datafn)
...
h_mask = h<200
temp = da['temp'].values[0,-1,:,:]
temp_ma = ma.masked_where(~(goa_mask&h_mask),temp)
...
p=ax0.pcolormesh(lon,lat,temp_ma,shading='nearest',cmap=cmo.cm.thermal,vmin=0,vmax=18)
...
efun.dar(ax0)
ax0.text(0.9,.1,'SST (°C)\n'+pd.to_datetime(da['ocean_time'].values[0]).strftime("%b %Y"),transform=ax0.transAxes,fontsize=12,zorder=600,va='bottom',ha='right')
