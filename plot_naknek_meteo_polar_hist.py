...
for i in range(nyeargroups):
    ...
    gg = df[df['year'].isin(yeargroup)][["sknt","rad"]]
    ...
    p=ax.pcolormesh(theta, R, histm.T, cmap=alpharainbow,vmin=0,vmax = 0.01)
    ...
    ax.text(1.3,1.1,'{}-{}'.format(yeargroup[0],yeargroup[-1]),transform=ax.transAxes,ha='right',color=rainbow(i),fontweight='bold')
    ax.set_rmax(50)
